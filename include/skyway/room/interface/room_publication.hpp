...
public:
    ...
    public:
        ...
        virtual void OnConnectionStateChanged(const ConnectionState new_state) {}
    };
    virtual ~RoomPublication() = default;
    /// @brief Idを取得します。
    virtual std::string Id() = 0;
    /// @brief PublicationのTypeを取得します。
    virtual domain::PublicationType Type() = 0;
    ...
    virtual std::vector<Codec> CodecCapabilities() = 0;
    /// @brief このPublicationのエンコーディング設定の一覧を取得します。
    virtual std::vector<Encoding> Encodings() = 0;
    /// @brief State(公開状態がEnableかDisabelかCancelか)を取得します。
    virtual PublicationState State() = 0;
    /// @brief Publisherが持つStreamを取得します。
    virtual std::shared_ptr<media::stream::interface::local::LocalStream> Stream() = 0;
    /// @brief イベントを購読します。
    ...
    virtual bool UpdateEncodings(std::vector<Encoding> encodings) = 0;
    /// @brief 公開しているStreamを変更します。
    virtual bool ReplaceStream(
        std::shared_ptr<media::stream::interface::local::LocalStream> stream) = 0;
    /// @deprecated 本機能は非推奨です。
    /// @brief 統計情報を取得します。
    /// @details 試験的なAPIです。今後インターフェースや仕様が変更される可能性があります。
    /// @param selector 取得対象のmemberID
    [[deprecated]] virtual std::optional<WebRTCStats> GetStats(const std::string& selector) = 0;
