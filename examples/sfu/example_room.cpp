...
bool ExampleRoom::JoinRoom(const std::string& room_name) {
    ...
    sfu_room_      = skyway::room::sfu::SFURoom::FindOrCreate(room_init);
    if (!sfu_room_) {
        std::cerr << "- [Error] room failed." << std::endl;
        return false;
    }
    // SFURoomにイベントリスナ(skyway::room::interface::Room::EventListenerの実装)を登録します。
    sfu_room_->AddEventListener(this);

    // SFURoomの情報を出力します。
    std::cout << "# Room" << std::endl;
    if (sfu_room_->Name()) {
        std::cout << "- Name: " << sfu_room_->Name().value() << std::endl;
    }
    std::cout << "- Id: " << sfu_room_->Id() << std::endl;

    // SFURoomに既にいるメンバーの一覧を表示します。
    std::cout << "- Room Members" << std::endl;
    for (auto& members : sfu_room_->Members()) {
        std::cout << "  - Id: " << members->Id() << std::endl;
    }

    // SFURoomにメンバーを入室させます。
    skyway::room::interface::RoomMemberInitOptions room_options;
    room_member_ = sfu_room_->Join(room_options);
    if (!room_member_) {
        std::cerr << "- [Error] sfu room join failed." << std::endl;
        return false;
    }
...
void ExampleRoom::SubscribeAll() {
    for (auto& publication : sfu_room_->Publications()) {
...
bool ExampleRoom::LeaveRoom() {
    ...
    // SFURoomに紐づくイベントリスナの登録を解除します。
    if (sfu_room_) {
        sfu_room_->RemoveEventListener(this);
    }
    // 各threadの終了を待ちます。
